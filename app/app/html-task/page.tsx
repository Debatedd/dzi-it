...
function colorMatch(computed: string, expected: string): boolean {
  return normalizeColor(computed) === expected.toLowerCase();
}

function pxClose(computed: string, expected: number, tol = 2): boolean {
  return Math.abs(parseFloat(computed) - expected) <= tol;
}

function pctClose(computed: string, containerW: number, pct: number, tol = 3): boolean {
  return Math.abs(parseFloat(computed) - (containerW * pct) / 100) <= tol * (containerW / 100);
}

function runChecks(doc: Document, containerW: number): CheckResult[] {
  const r: CheckResult[] = [];
...
export default function HtmlTaskPage() {
  ...
  function handleCheck() {
    ...
    iframe.onload = () => {
      ...
      if (!doc) return;
      setResults(runChecks(doc, iframe.clientWidth || 1200));
      setTab("task");
    };
  }
  ...
  return (
    <main className="min-h-screen flex flex-col" style={{ background: "var(--bg)", color: "var(--text)" }}>
      <div className="flex items-center justify-between px-6 py-3 border-b" style={{ background: "var(--nav-bg)", borderColor: "var(--border)" }}>
        ...
        <span className="font-bold">Задача 28 — HTML Кодиране</span>
        ...
        {tab === "task" && (
          <div className="flex-1 overflow-y-auto p-6" style={{ maxWidth: 720, margin: "0 auto" }}>
            <div className="glass rounded-2xl p-6" style={{ border: "1px solid var(--border)" }}>
              <h2 className="font-bold text-lg mb-4" style={{ color: "var(--accent)" }}>Задача 28</h2>
              <p className="mb-4 text-sm" style={{ color: "var(--muted)" }}>Създайте уеб страница за информационна кампания с HTML и CSS.</p>
              <ol className="space-y-2 text-sm list-decimal list-inside" style={{ color: "var(--text)" }}>
                <li><b>Заглавие в браузъра</b> — &bdquo;Рециклиране&ldquo;</li>
                <li><b>Блок 1</b> — ширина 80%, шрифт Georgia, съдържа всички останали елементи</li>
                <li><b>Блок 2 (header)</b> — височина 90px, фон #2E7D32, padding 20px. Лого (logo.png) вляво, ширина 8%. Навигация вдясно, вертикално центрирана: цвят #ffffff, без подчертаване, размер 20px, word-spacing 10px, margin top/bottom 30px.</li>
                <li><b>Блокове 3 и 4</b> — наредени един до друг, ширина 50%, височина 400px, фон #1B5E20. Блок 3: заглавие + 2 параграфа. Блок 4: изображение (blue_green_yellow.png, ширина 50%) + заглавие.</li>
                <li><b>Блок 5</b> — височина 500px, фон #e8f5e9. Заглавие, текст и четири еднакви блока (6-9) наредени един до друг.</li>
                <li><b>Блокове 6, 7, 8, 9</b> — ширина 25%, височина 400px, фон #e8f5e9. Изображение (ширина 50%), заглавие, списък (само блок 6), текст (цвят #1B5E20, 15px, italic).</li>
                <li><b>Блок 10 (footer)</b> — височина 50px, фон #1B5E20. Текст: центриран, бял, 16px, padding-top 20px.</li>
